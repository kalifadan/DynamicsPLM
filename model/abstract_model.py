...
class AbstractModel(pl.LightningModule):
    def __init__(self,
        ...
        # TODO: ADD SAVE PARAMS
        # Project SHP vectors (20D) to hidden_dim for compatibility with MHA
        self.shp_proj = nn.Linear(20, self.model.config.hidden_size)

        # Cross-attention module: Protein (Q) attends to SHP (K, V)
    ...
    @abc.abstractmethod
    def loss_func(self, stage: str, outputs, labels, inputs=None, info=None) -> torch.Tensor:
        """
        ...
        Args:
            ...
            info: information about the proteins
    ...
    def training_step(self, batch, batch_idx):
        inputs, labels, dynamic_features, info = batch
        outputs = self(**inputs, dynamic_features=dynamic_features)
        loss = self.loss_func('train', outputs, labels, inputs, info=info)
        return loss
    
    def validation_step(self, batch, batch_idx):
        inputs, labels, dynamic_features, info = batch
        outputs = self(**inputs, dynamic_features=dynamic_features)
        loss = self.loss_func('valid', outputs, labels, inputs, info=info)
        return loss

    def test_step(self, batch, batch_idx):
        inputs, labels, dynamic_features, info = batch
        outputs = self(**inputs, dynamic_features=dynamic_features)
        loss = self.loss_func('test', outputs, labels, inputs, info=info)
        return loss
    ...
    def load_checkpoint(self, from_checkpoint, load_prev_scheduler):
        ...
        for key, module in [
            ...
            ("sha_proj", self.shp_proj),
            ("shp_embedding_layer", self.shp_embedding_layer)
        ]:
            if key in state_dict:
                print(f"loading {key} for the model")
