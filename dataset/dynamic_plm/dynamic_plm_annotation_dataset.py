...
class SaprotAnnotationDataset(LMDBDataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            data = self.last_entry if self.last_entry else json.loads(self._get(random.randint(0, len(self) - 1)))

        seq = data['seq']
        dynamic_features = {}

        if "shp" in data:
            dynamic_features["shp"] = data["shp"]
        else:
            print("Found empty shp values for protein: ", data["name"])
