...
@register_dataset
class DynamicPLMRegressionDataset(LMDBDataset):
	def __init__(self,
	             tokenizer: str,
	             max_length: int = 1024,
	...
	def __getitem__(self, index):
		...
		label = entry['fitness']
		return seq, label, dynamic_features, None
	
	def __len__(self):
		return int(self._get("length"))
	
	def collate_fn(self, batch):
		seqs, labels, dynamic_features, information_list = tuple(zip(*batch))
		...
		return inputs, labels, dynamic_features[0], information_list
